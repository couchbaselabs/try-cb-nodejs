...
module.exports.findAll = function (from, to, leave,done) {
    ...
    db.query(queryPrep, function (err, res) {
        ...
        if (res) {
            ...
            queryPrep="SELECT a.name, s.flight, s.utc, r.sourceairport, r.destinationairport, r.equipment, " +
            "ROUND(r.distance* "+config.application.distanceCostMultiplier+") AS price, ROUND(r.distance/"+
            config.application.avgKmHr + ") AS flighttime FROM `" + config.couchbase.bucket + "` r UNNEST r.schedule s JOIN `" +
 ...
 * @returns {Number}
 */
function getArrival(dateStr,distance){
    var hrs=Math.round(distance/config.application.avgKmHr);
    var d= new Date(Date.parse(dateStr));
    return parseInt(d.getHours()+hrs);
}